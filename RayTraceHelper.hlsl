...
float2 getTexCoord(Triangle tri, float3 pt)
{
	...
	float3 vector0 = tri[0].position - pt;
	float3 vector1 = tri[1].position - pt;
	float3 vector2 = tri[2].position - pt;

	// calculate areas as well as factors

	float a0 = magnitude(cross(tri[0].position - tri[1].position,
		tri[0].position - tri[2].position)); // main triangle area
	...
	// find the uv coord
	return tri[0].texcoord * a1 + tri[1].texcoord * a2 + tri[2].texcoord * a3;
}
