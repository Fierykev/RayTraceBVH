...
int main()
{
	...
	cout << maximizeLoop << endl;

	float3 minData = nodes[0].bbMin;
	float3 maxData = nodes[0].bbMax;

	for (uint i = 1; i < numObjects; i++)
	{
		minData = min(minData, nodes[i].bbMin);
		maxData = max(maxData, nodes[i].bbMax);
	}

	cout <<
		nodes[numObjects].bbMax.x << ", " <<
		nodes[numObjects].bbMax.y << ", " <<
		nodes[numObjects].bbMax.z << " | " <<
		nodes[numObjects].bbMin.x << ", " <<
		nodes[numObjects].bbMin.y << ", " <<
		nodes[numObjects].bbMin.z << "| " <<
		maxData.x << ", " <<
		maxData.y << ", " <<
		maxData.z << "| " <<
		minData.x << ", " <<
